...
This is `comradarr-dev`, a Python CLI tool for Comradarr development workflows. It manages PostgreSQL databases, runs tests, and provides a dev server lifecycle with isolated databases. Built with Typer for CLI and Textual for TUI.
...
# Run CLI directly
uv run comradarr-dev --help

# Run the interactive TUI menu
uv run comradarr-dev menu
```
...
### CLI Structure (`src/comradarr_dev/`)
...
- **state.py** - State persistence via `/tmp/comradarr-dev-state.json` and credentials in `.comradarr-dev-dbs.json`
