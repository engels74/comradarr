<script lang="ts">
	...
	const stateConfig: Record<QueueState, { bg: string; text: string; label: string; animate?: boolean }> = {
		...
		searching: { bg: 'bg-yellow-500/20', text: 'text-yellow-600 dark:text-yellow-400', label: 'Searching', animate: true },
		cooldown: { bg: 'bg-orange-500/20', text: 'text-orange-600 dark:text-orange-400', label: 'Cooldown' },
		exhausted: { bg: 'bg-red-500/20', text: 'text-red-600 dark:text-red-400', label: 'Exhausted' }
	};

	const config = $derived(
		state && state in stateConfig
			? stateConfig[state as QueueState]
			: stateConfig.pending
	);
</script>
