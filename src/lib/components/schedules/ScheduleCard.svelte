...
const typeColors: Record<string, string> = {
	incremental: 'bg-blue-500/10 text-blue-600 dark:text-blue-400',
	full_reconciliation: 'bg-purple-500/10 text-purple-600 dark:text-purple-400'
};
			...
			<div class="space-y-1">
				...
				<div class="flex items-center gap-2 flex-wrap">
					...
					{#if schedule.connector}
						<span
							class="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium bg-gray-500/10 text-gray-600 dark:text-gray-400"
						>
							{schedule.connector.name}
						</span>
					{:else}
						<span
							class="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium bg-green-500/10 text-green-600 dark:text-green-400"
						>
							All Connectors
						</span>
					{/if}
