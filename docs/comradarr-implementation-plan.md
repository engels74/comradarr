...
- [x] **Architectural decision: flat package layout over `domain/<bounded_context>/`.** Backend rules §7 prescribes a domain-bundled structure (`domain/<context>/{controllers,services,repositories,schemas}.py`). Comradarr v1 is a single bounded context with several cross-cutting subsystems (connectors, rotation, security, notifications). Bundling all subsystems under one `domain/comradarr/` directory adds a redundant nesting level without separation benefit. The plan therefore uses concern-flat packages: `comradarr/api/controllers/`, `comradarr/services/<subsystem>/`, `comradarr/repositories/`, `comradarr/connectors/`, `comradarr/db/{base,models}.py`. Backend rules §7 is treated as guidance; this divergence is intentional and scoped to v1.
...
- [ ] `server.ts` exports `createServerClient(event)` returning `createClient<paths>({ baseUrl: '', fetch: event.fetch })` (RULE-OAPI-002 + RULE-OAPI-003 — server-side calls thread `event.fetch` so cookies / headers propagate, and the typed paths binding is identical to the browser client).
