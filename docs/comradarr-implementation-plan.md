...
- [ ] Ensure the SvelteKit `hooks.server.ts` forwards the user's original `Origin` on backend calls so the backend's check sees the genuine browser origin (PRD §16, "SvelteKit Form Actions"). **SvelteKit-side posture (frontend RULE-SEC-001):** SvelteKit's built-in CSRF protections stay on; `kit.csrf.checkOrigin` is **never** set to `false`. The deploy-time `ORIGIN` env-var is set to the canonical public origin, and any external trusted origins are added to `kit.csrf.trustedOrigins` rather than disabling the check. The Litestar Origin/Referer comparator above is a defense-in-depth backstop for the SvelteKit guard, not a replacement.
...
- [ ] Build an `httpx.AsyncClient` factory (RULE-ASYNC-001 — async-only outbound I/O; RULE-HTTP-001 — `httpx.AsyncClient` is the only sanctioned outbound HTTP primitive, sync `requests`/`urllib3` are forbidden) that:
  ...
  - [ ] applies explicit timeouts — `httpx.Timeout(connect=5.0, read=30.0, write=10.0, pool=5.0)` totalling at most 60 s wall-clock per request — and a per-host bounded connection pool `httpx.Limits(max_connections=10, max_keepalive_connections=5)` (RULE-HTTP-002 / RULE-HTTP-003); the streaming reader additionally enforces a **per-chunk idle-byte timeout of 5 s** so a slow-loris upstream that sends one byte per ~30 s (under the read-timeout's reset clock) cannot tie up a connection indefinitely — every gap between received bytes longer than 5 s aborts the stream with a transient connector error.
...
- [ ] `scripts/gen-api.ts` runs `openapi-typescript http://localhost:8000/api/schema/openapi.json -o src/lib/api/schema.d.ts` (**RULE-OAPI-001** — consume Litestar's OpenAPI surface at the canonical `/api/schema` mount from §5.1.4) and emits the file with **`@ts-nocheck`/checksum header** so **RULE-OAPI-002** (generated types are machine-owned — never hand-edited) is enforceable as a CI lint: a pre-commit / CI step diffs the regenerated file against the committed copy and fails on any drift, so a developer who hand-edits `schema.d.ts` is caught at PR time. Per **RULE-AUTHZ-MATCH-001** the openapi-fetch wrapper never silently coerces a non-2xx response into success — every consumer must check the discriminated `data | error` union before reading; a "no-error-field-so-it-must-be-fine" default is forbidden.
- [ ] `client.ts` exports `createBrowserClient()` returning `createClient<paths>({ baseUrl: '' })` (RULE-OAPI-002 — typed `createClient<paths>()` with discriminated `{ data, error }` reads on every call site).
- [ ] `server.ts` exports `createServerClient(event)` returning `createClient<paths>({ baseUrl: '', fetch: event.fetch })` (**RULE-OAPI-002** + **RULE-OAPI-003** + **RULE-SK-FETCH-001** — load functions and server endpoints use the framework-provided `event.fetch`, never global `fetch`, so cookies / headers propagate, relative URLs work during SSR, and internal `+server.ts` calls coalesce without an HTTP round-trip; the typed paths binding is identical to the browser client). Per RULE-SK-FETCH-001 a global `fetch(...)` call inside any `+*.server.ts` or `+server.ts` is a defect — code review and an ast-grep gate (registered in §5.23) reject it.
