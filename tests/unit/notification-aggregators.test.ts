...
describe('buildAggregatePayload', () => {
	...
	describe('search_exhausted aggregation', () => {
		it('should aggregate multiple search exhausted events', () => {
			...
			expect(payload.fields?.some(f => f.name === 'Total Attempts' && f.value === '8')).toBe(true);
		});
	...
	describe('sweep_started aggregation', () => {
		it('should aggregate multiple sweep started events', () => {
			const entries = [
				createHistoryEntry('sweep_started', {
					connectorId: 1,
					connectorName: 'Sonarr',
					sweepType: 'gap'
				}, 1),
			...
			expect(payload.fields?.some(f => f.name === 'Sweeps Started' && f.value === '2')).toBe(true);
		});
	...
	describe('sweep_completed aggregation', () => {
		it('should sum totals across multiple sweeps', () => {
			const entries = [
				createHistoryEntry('sweep_completed', {
					connectorId: 1,
					connectorName: 'Sonarr',
					gapsFound: 10,
					upgradesFound: 5,
			...
			expect(payload.fields?.some(f => f.name === 'Upgrades Found' && f.value === '8')).toBe(true);
	...
	describe('sync_completed aggregation', () => {
		it('should sum processed items across syncs', () => {
			const entries = [
				createHistoryEntry('sync_completed', {
					connectorId: 1,
					connectorName: 'Sonarr',
					itemsProcessed: 100,
					itemsCreated: 10,
			...
			expect(payload.fields?.some(f => f.name === 'Items Processed' && f.value === '150')).toBe(true);
	...
	describe('sync_failed aggregation', () => {
		it('should list all failures with errors', () => {
			const entries = [
				createHistoryEntry('sync_failed', {
					connectorId: 1,
					connectorName: 'Sonarr',
			...
			expect(payload.fields?.some(f => f.name === 'Total Failures' && f.value === '2')).toBe(true);
	...
	describe('connector_health_changed aggregation', () => {
		it('should list all health changes', () => {
			const entries = [
				createHistoryEntry('connector_health_changed', {
					connectorId: 1,
					connectorName: 'Sonarr',
	...
	describe('edge cases', () => {
		...
		it('should truncate long content titles', () => {
			const entries = [
				createHistoryEntry('search_success', {
					contentTitle: 'This is a very very very long title that should be truncated for readability',
					quality: 'HDTV-1080p',
					connectorName: 'Sonarr'
				})
			];
