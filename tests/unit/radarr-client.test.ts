	...
	it('should detect Radarr v5 version', async () => {
		globalThis.fetch = createMockFetch(
			vi
				.fn()
				.mockResolvedValue(
					mockJsonResponse({ appName: 'Radarr', version: '5.2.0.8171', isDocker: true })
				)
		);
		const client = new RadarrClient(validConfig);
	...
	it('should detect Radarr v4 version', async () => {
		globalThis.fetch = createMockFetch(
			vi
				.fn()
				.mockResolvedValue(
					mockJsonResponse({ appName: 'Radarr', version: '4.7.0.7654', isDocker: true })
				)
		);
		const client = new RadarrClient(validConfig);
		const result = await client.detectApiVersion();
	...
	it('should detect Radarr v3 version', async () => {
		globalThis.fetch = createMockFetch(
			vi
				.fn()
				.mockResolvedValue(
					mockJsonResponse({ appName: 'Radarr', version: '3.2.2.5080', isDocker: true })
				)
		);
		const client = new RadarrClient(validConfig);
		const result = await client.detectApiVersion();
	...
	it('should default to v3 for invalid version strings', async () => {
		globalThis.fetch = createMockFetch(
			vi
				.fn()
				.mockResolvedValue(
					mockJsonResponse({ appName: 'Radarr', version: 'invalid-version', isDocker: true })
				)
		);
		const client = new RadarrClient(validConfig);
		const result = await client.detectApiVersion();
	...
	it('should call /api/v3/system/status endpoint', async () => {
		...
		globalThis.fetch = createMockFetch(
			vi.fn().mockImplementation(async (url: string) => {
				capturedUrl = url;
				return mockJsonResponse({ appName: 'Radarr', version: '5.0.0' });
			})
		);
		const client = new RadarrClient(validConfig);
